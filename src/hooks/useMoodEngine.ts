...
function computeCuriosity(appCategory: AppCategory): number {
  switch (appCategory) {
    ...
    case "browsing":      return 60;
    case "communication": return 55;
...
// OS idle window that triggers a yawn (3–5 min)
const YAWN_IDLE_MIN     = 3;
const YAWN_IDLE_MAX     = 5;
...
export function useMoodEngine({
  ...
  appCategory,
  petState,
}: UseMoodEngineOptions): { moodOverride: string | null } {
  ...
  useEffect(() => { appCategoryRef.current = appCategory; }, [appCategory]);
