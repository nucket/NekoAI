...
export function usePetBrain({
  appCategory,
  idleMinutes,
}: DesktopContextResult): PetBrainResult {
  ...
  useEffect(() => {
    ...
    if (appCategory === "coding") {
      if (codingStartRef.current === null) {
        ...
        !codingAlertedRef.current &&
        now - codingStartRef.current >= CODING_ALERT_MS
      ) {
        codingAlertedRef.current = true;
        const msg =
          CODING_MESSAGES[Math.floor(Math.random() * CODING_MESSAGES.length)];
